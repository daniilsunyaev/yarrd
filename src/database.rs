...
impl Database {
    ...
    pub fn execute(&mut self, command: Command) -> Result<Option<Vec<Row>>, String> {
        match command {
    ...
    fn create_table(&mut self, table_name: SqlValue, columns: Vec<ColumnDefinition>) -> Result<Option<Vec<Row>>, String> {
    ...
    fn drop_table(&mut self, table_name: SqlValue) -> Result<Option<Vec<Row>>, String> {
        let table_name_string = table_name.to_string();
    ...
    fn select_rows(&self, table_name: SqlValue, column_names: Vec<SelectColumnName>, where_clause: Option<WhereClause>) -> Result<Option<Vec<Row>>, String> {
        let table_name_string = table_name.to_string();
    ...
    fn insert_rows(&mut self, table_name: SqlValue, column_names: Option<Vec<SqlValue>>, values: Vec<SqlValue>) -> Result<Option<Vec<Row>>, String> {
        let table_name_string = table_name.to_string();
    ...
    fn update_rows(&mut self, table_name: SqlValue, field_assignments: Vec<FieldAssignment>, where_clause: Option<WhereClause>) -> Result<Option<Vec<Row>>, String> {
        let table_name_string = table_name.to_string();
        let table = match self.tables.get_mut(table_name_string.as_str()) {
            None => return Err(format!("table '{}' does not exist", table_name_string)),
            Some(existing_table) => existing_table,
    ...
    fn delete_rows(&mut self, table_name: SqlValue, where_clause: Option<WhereClause>) -> Result<Option<Vec<Row>>, String> {
        let table_name_string = table_name.to_string();
        let table = match self.tables.get_mut(table_name_string.as_str()) {
            None => return Err(format!("table '{}' does not exist", table_name_string)),
            Some(existing_table) => existing_table,
